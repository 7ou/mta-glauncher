...
function canHeFire(player)
	...
	if consumeNades and not ((getPedWeapon(player, 8) == 16) and (getPedTotalAmmo(player, 8) > 0)) then return false end
...
function fireNade()
	if canHeFire(localPlayer) then
		if not reloadStarted then
			...
			outputChatBox("You can't fire while reloading!", 255, 255, 0)
		end
	end
end
